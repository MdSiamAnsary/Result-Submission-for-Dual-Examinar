<?php
	...
	if(isset($_POST['btn-register']))
	{
        ...
        //---------------------------------------------------------------------------------------------
        // Name can only have alphabets and whitespaces 

        if(empty($fullname)){
            $error = true;
            $errorFullname = "Please input your name";
            $regMsg = 'Registration unsuccessful';
        }
        else
        {
            $field = filter_var(trim($fullname), FILTER_SANITIZE_STRING);
            if(filter_var($field, FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z\s]+$/"))))
            {
               $fullname = $field;
            } 
            ...
            {
                $error = true;
                $errorFullname = "Please input a valid name";
                $regMsg = 'Registration unsuccessful';
            }
        }
        ...
        $sql = "select * from  teachers_table where username='$username' ";
        $result = mysqli_query($conn, $sql);
        $count = mysqli_num_rows($result);
        $row = mysqli_fetch_assoc($result);
        if($count>=1)
    	...
    	if(!$error){
			$sql = "insert into  teachers_table(name, username, email ,password) values('$fullname', '$username', '$email', '$password')";
...
<html lang="en">
	...
	<title>Result Submission for Dual Examinar</title>
	...
	<div class="container-fluid">
		<div class="container">
			<h2 class="text-center" id="title" style="font-family: Garamond; font-size: 2.5em; font-style: bold;">
				<BR>
                	...
                	?>
						...
						<fieldset>							
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="registerfullname" id="registerfullname" class="form-control" 
